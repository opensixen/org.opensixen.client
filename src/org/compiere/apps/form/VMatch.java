	...
	implements FormPanel, ActionListener, TableModelListener, ListSelectionListener
{
	...
	private void cmd_searchTo()
	{
		...
		else
		{
			...
			BigDecimal matchedQty = (BigDecimal) xMatchedTable.getValueAt(row, I_MATCHED);
			qty = docQty.subtract(matchedQty);
			...
			if (id != null && id.isSelected())
			{
				...
				else
				{
					if (matchMode.getSelectedIndex() == MODE_NOTMATCHED)
						qty = qty.add((BigDecimal)xMatchedToTable.getValueAt(row, I_QTY));  //  doc
